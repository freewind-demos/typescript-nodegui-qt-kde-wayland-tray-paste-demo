...
- X11：`clipboard + xdotool + Shift+Insert / Ctrl+V / Ctrl+Shift+V`
- Wayland：`clipboard + ydotool + Shift+Insert / Ctrl+V / Ctrl+Shift+V`
...
- 这个 demo 不是“直接向别的进程注入文本”，而是“先写剪贴板，再模拟粘贴快捷键”。
...
3. 点击某一项后：
   - 先把该句写入 Qt 剪贴板。
   - X11 下依次调用 `xdotool key --clearmodifiers Shift+Insert`、`Ctrl+V`、`Ctrl+Shift+V`。
   - Wayland 下依次调用 `ydotool` 模拟 `Shift+Insert`、`Ctrl+V`、`Ctrl+Shift+V`。
