...
当前实现优先服务“验证 Shift+Insert 是否通”这个目标，所以同时支持 `KDE + X11` 和 `KDE + Wayland`。

- X11：`primary selection + xdotool + Shift+Insert`
...
3. 点击某一项后：
   ...
   - X11 下调用 `xdotool key --clearmodifiers Shift+Insert`。
   - Wayland 下用 `wl-copy --primary` 写入，再调用 `ydotool` 模拟 `Shift+Insert`。
